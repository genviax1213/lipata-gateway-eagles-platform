...
class MemberRegistrationController extends Controller
{
    ...
    private function consumeGoogleClaim(Request $request, string $email): ?array
    {
        ...
        $claim = GoogleOAuthClaimStore::consume(GoogleOAuthClaimStore::INTENT_MEMBER_REGISTRATION, $token);
        if (!$claim) {
        ...
        if ($this->normalizeEmail((string) $claim['email']) !== $email) {
            throw ValidationException::withMessages([
                'email' => ['Google verified email must match the registration email.'],
            ]);
        }
    ...
    private function createVerifiedMemberFromRegistration(MemberRegistration $registration): void
    {
    ...
    public function register(Request $request)
    {
        ...
        $plainToken = $googleClaim ? null : VerificationToken::generate();
        ...
        $registration = MemberRegistration::query()->create([
            ...
            'status' => $googleClaim ? MemberRegistration::STATUS_COMPLETED : MemberRegistration::STATUS_PENDING_VERIFICATION,
            'verification_token' => $plainToken ? hash('sha256', $plainToken) : null,
            'email_verified_at' => $googleClaim ? now() : null,
            'completed_at' => $googleClaim ? now() : null,
        ]);

        if ($googleClaim) {
            $this->createVerifiedMemberFromRegistration($registration);

            return response()->json([
                'message' => 'Google verified your email. Your member account is now active.',
                'registration_id' => $registration->id,
            ], 201);
        }

        $registration->notify(new MemberRegistrationVerificationToken($plainToken, $normalizedEmail));
