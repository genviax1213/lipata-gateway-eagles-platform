...
export default function FormalPhotoStaffViewer({
  ...
  const runSearch = async () => {
    ...
    try {
      const response = await api.get<PaginatedMembersResponse>("/formal-photos/members", {
        params: {
          page: 1,
          search: search.trim(),
        },
      ...
      setMembers(rows);
      if (rows.length === 0) {
        onNotice("No members matched the current search.");
      }
    } catch {
      onError("Unable to search members for the formal-photo viewer.");
    } finally {
      setSearching(false);
    }
  };
  ...
  const selectMember = async (member: FormalPhotoMemberRow) => {
    ...
    try {
      const response = await api.get<StaffFormalPhotoResponse>(`/members/${member.id}/formal-photo`);
      setSelectedMember(response.data.member ?? member);
      setFormalPhoto(response.data.formal_photo ?? null);
      if (!response.data.formal_photo) {
        onNotice("This member does not have a saved formal photo yet.");
      }
    } catch {
      onError("Unable to load the selected member's formal photo.");
    } finally {
    ...
    <div className="rounded-xl border border-white/20 bg-white/10 p-4">
      <div className="mb-4">
        ...
        <p className="mt-1 max-w-3xl text-sm text-mist/80">
          Search a member, select the record, and preview the private formal photo through the authenticated backend endpoint.
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(320px,0.9fr)]">
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-white/15 bg-navy/35 p-4">
            <div className="mb-3 flex items-center justify-between gap-2">
              <h3 className="font-heading text-lg text-offwhite">Matching Members</h3>
            ...
            <div className="max-h-96 space-y-2 overflow-y-auto pr-1">
              {members.map((member) => (
                ...
                >
                  ...
                  <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-mist/75">
                    <span>Member No: {member.member_number}</span>
              ...
              {members.length === 0 ? (
                <div className="rounded-lg border border-dashed border-white/15 px-4 py-8 text-center text-sm text-mist/70">
                  Search for a member to load the private formal-photo preview.
                </div>
              ) : null}
            </div>
          </div>
        ...
        <div className="rounded-xl border border-white/15 bg-navy/40 p-4">
          <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
            <div>
              <h3 className="font-heading text-lg text-offwhite">Selected Member Preview</h3>
              <p className="text-xs text-mist/75">
                {selectedMember
                  ? `${memberLabel(selectedMember)} · ${selectedMember.member_number}`
                ...
                <div className="flex h-full items-center justify-center px-6 text-center text-sm text-mist/70">
                  {loadingPhoto
                    ...
                    : previewFailed
                      ...
                      : selectedMember
                        ? "This member has no saved formal photo yet."
                        : "Select a member from the search results to preview the private formal photo."}
                </div>
              )}
            </div>
          </div>
