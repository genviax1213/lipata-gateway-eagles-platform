...
export default function PortalDashboard() {
  ...
  const recoverPendingVerificationApplicant = useCallback(async () => {
    ...
    if (!window.confirm(`Delete the wrong-email pending verification record for ${appName(selectedApplication)} so they can register again using the correct email address?`)) {
      return;
    }
    ...
    try {
      ...
      setNotice(res.data?.message ?? "Pending verification applicant deleted.");
